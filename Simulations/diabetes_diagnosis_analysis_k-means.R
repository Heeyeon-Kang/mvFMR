...
wssplot <- function(data, nc = 15, seed = 1234) {
  ...
  plot(1:nc, wss, type="b", xlab = "Number of clusters",
       ylab = "Within groups sum of squares", main = "k-means clustering of diabetes diagnosis data")
}
wssplot(Y) # K=4 is the optimal K

set.seed(1032)
group <- kmeans(Y, centers=4)$cluster
...
for(i in 1:nrow(Y)){
  ...
  }else if(dat_Y$group[i] == 3){
    dat_Y$group[i] <- "Group 2"
  }else{
    dat_Y$group[i] <- "Group 4"
  }
}
